...
that average.  The corresponding identity-coordinate final route can now be
stated directly from canonical \(j=1\) equality, nonpositive target
environment, and nonnegative \(\Theta\)-source average.  For nonzero labels,
Lean proves the complementary bound: the
...
nonnegativity of the upper bound.  On the nonzero-label part, the formal
...
canonical \(j=1\) Gaussian components.  Lean also proves that, in the
nonpositive environment branch, the nonzero coordinate target-bound formulation
is equivalent to this environment bound against the \(\Theta\)-source average;
...
\(j=1\) equality and the nonzero Gaussian target-bound predicate.
In the all-label branch, the analogous direct endpoint uses canonical \(j=1\)
equality, nonpositive target environment, and nonnegative \(\Theta\)-source
average.
The
