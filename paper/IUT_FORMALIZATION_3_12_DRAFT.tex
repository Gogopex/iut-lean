...
environment, and nonnegative \(\Theta\)-source average; if the all-label
Gaussian target bound is supplied directly, the same endpoint no longer needs
the nonpositive/nonnegative branch hypotheses.  For nonzero labels, Lean proves
the complementary bound: the absolute exponent is at least \(1\), so a
