...
\item proof that, for nonzero \(\deg(q)\), one label-independent scale cannot
  ...
  converse classifier: such a scale exists if and only if \(\deg(q)=0\);
...
\item the Step (xi-f) final algebra:
  ...
  \(-|\log(q)|=-|\log(\Theta)|\) and \(-|\log(\Theta)|<0\); hence the endpoint
  splits into the boundary branch \(C_\Theta=-1\) or the strict branch
  \(C_\Theta>-1\);
