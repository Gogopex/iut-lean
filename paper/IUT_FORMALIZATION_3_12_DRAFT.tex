...
\item the degree-level Gaussian evaluation
  ...
  torsor action is transported by coordinate addition.  It also records the
...
\item the Step (x) to Step (xi) upper-ray handoff:
  ...
  the hull/determinant upper-ray datum used in Step (xi); Lean now exposes the
...
\item the Step (x) to Step (xi-g) two-computation handoff:
  ...
  and hence that the pre-indeterminacy average is the hull-side q-reading;
...
\item the Step (xi-h) warning: for \(N\ge2\) and \(\Theta<0\), the tensor-power
  ...
  upper ray, so the replacement changes the comparison region;
\item the Step (xii) shadow: local Frobenioid \(p_v^N\)-ambiguity changes
  ...
  values agree, exactly when the local exponent-step product is zero;
...
\item the Remark 3.12.1(ii) generalized-link numerical bound
  ...
  the resulting strict conclusion \(-1<C_\Theta\) in this case;
\item the \(q^\lambda\)-pilot endpoint algebra:
  ...
  \(-1<C_\Theta\) conclusion when \(\lambda<1\);
\item the Remark 3.12.2 distinct-label implication
  ...
  \Rightarrow\Theta\text{-itw}/\mathrm{indets}\) as formal projection;
\item the Remark 3.12.2 toy calculation: forgotten concrete assignments
  ...
  \(-h\le-2h+\epsilon\), and the bound \(h\le\epsilon\);
\item the Fig. 3.9 column table: Lean packages the upper-row similarities
  ...
  log-Kummer non-interference; it also packages the lower-row differences:
...
\item the Remark 3.12.2(v) absorption distinction: zero-column unit
  ...
  outcomes together with the corresponding Fig. 3.9 log-shell treatments;
...
rejects collapse of the q-native and \(\Theta\)-native labels.
...
log-shell treatment distinguish the 0-column from the 1-column.
The absorption flag records the zero-column upper-bound versus one-column
equality distinction, tied to the two Fig. 3.9 log-shell treatments.
...
the underlying \(F_\ell\)-label model and additive torsor laws are now packaged
as source-side endpoints.
...
The continuation point remains the finite \(q^{j^2}\) branch and the
distinct-label/intertwining corridor.  The next significant formal step is not
another wrapper.  It is to replace current explicit Hodge/SHE transport
assumptions by constructions closer to the papers:
