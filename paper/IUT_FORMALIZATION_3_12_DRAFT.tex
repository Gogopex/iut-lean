...
\item the Step (xii) shadow: local Frobenioid \(p_v^N\)-ambiguity changes
  ...
  collapse to both fixed q-readings; Lean also combines this local shift
  calculation with the upper-semi quotient audit: outside the collapsed subset,
  quotient equality of two shifted local readings is equivalent to equality of
  their integer exponents, assuming nonzero local step; in the boundary branch,
  if the quotient collapses only the fixed q-reading, a nonzero local shift
  remains distinct from the quotient images of both the q-reading and the
  determinant reading; conversely, if an arbitrary collapsed region contains
  the fixed q-boundary, Lean proves the exact audit criterion: quotient equality
  between a local shifted value and q is equivalent to membership of that
  shifted value in the collapsed region.  For the concrete q-upper-ray collapse
  \(\{x\mid x\le q\}\), Lean proves the sign classification: a negative local
  shift term lies below q and is quotient-identified with q, a zero term equals
  q, and a positive term lies above q and is not quotient-identified with q.
  The same classification is now proved for the actual hull/determinant upper
  ray already present in the corridor, using the boundary identification
  \(q=\Theta_{\mathrm{hull}}\), and then packaged with the determinant boundary
  as a boundary-versus-strict endpoint theorem.  Lean also exposes the two
  nonzero sign cases separately: negative local shifts are genuinely below q
  but quotient-collapse to q in the actual upper ray, while positive local
  shifts remain quotient-separated from q.  Equivalently, in the boundary
  branch, actual-upper-ray quotient equality with q holds exactly when the
  local shift term is nonpositive; this exact threshold is now also packaged
  with the determinant boundary as a boundary-versus-strict endpoint theorem.
  The complementary separation criterion is also formalized: quotient
  non-equality with q is equivalent to a positive local shift term, again with
  a boundary-versus-strict endpoint package.  Under a positive local prime-step
  log-volume, Lean specializes this to the exponent: nonpositive exponents
  quotient-collapse to q, while positive exponents remain quotient-separated
  from q; the same criterion is now exposed directly for integer exponents.
  Hence, if the collapsed region also contains a nonzero local shift, Lean
  proves that the quotient identifies it with the fixed q/determinant boundary
  while the underlying log-volumes remain unequal; this broad-collapse
  diagnostic is now packaged as a
  boundary-versus-strict theorem together with the determinant boundary.  The
  same diagnostic applies to two shifted local readings with unequal exponents
  and nonzero local step when both lie in the collapsed subset;
...
\item the Remark 3.9.5(v) upper-semi set quotient:
  ...
  mechanism used to explain upper semi-compatibility; Lean also proves the
  converse audit: a nonempty subset maps to the collapsed singleton only if it
  was contained in \(S\), matching the expected equivalence relation in
  Remark 3.9.5(vi); moreover, any map between subsets contained in \(S\)
  becomes the identity on the collapsed quotient image;
