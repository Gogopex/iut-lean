...
\item a separate balanced sign-quotient theta-degree profile which does descend
  in the current finite model;
...
of raw representative \(j^2\) theta-degree data to \(|F_\ell|\); a balanced
sign-compatible replacement descends, but is not the final comparison level used
by the current 3.12 route.
...
\item the replacement of the present balanced sign-quotient control model by the
  exact IUT theta-value construction over \(|F_\ell|\);
