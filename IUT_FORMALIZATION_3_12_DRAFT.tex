...
\item a Gaussian-to-factored-SHE bridge:
  ...
  audit all the way to \(q_{\mathrm{signed}}\le\Theta_{\mathrm{signed}}\);
...
bound, so the zero label requires no target upper bound.
...
summand has weight \(0\).  The remaining task is to replace the current
degree-level Gaussian shadow by the actual Hodge--Arakelov Gaussian/Frobenioid
construction behind this environment comparison.
