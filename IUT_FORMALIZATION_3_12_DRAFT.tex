...
\item the log-volume shadow of the tensor packet over \(S^\pm_{j+1}\), with
  ...
  denominator \(j+1\), invariant under reindexing by container permutations;
...
\texttt{processionTensorPacketPermutationInvariant} & = \texttt{true},\\
\texttt{processionIndeterminacyFactorialAvailable} & = \texttt{true},\\
\texttt{absLabelProcessionExponentBridgeAvailable} & = \texttt{true},\\
