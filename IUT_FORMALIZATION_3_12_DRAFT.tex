...
\item the product over base valuations \(v_\mathbb{Q}\) of such tensor packets,
  with each direct-sum factor audited against its base place, and a log-volume
  shadow of the labeled \((F_{\mathrm{mod}}^\times)_j\)-action adding one unit
  contribution for each base-valuation factor;
...
packet layer, finite splitting-monoid action shadow, and finite
\((F_{\mathrm{mod}}^\times)_j\)-action shadow over base-valuation products.
...
\item the construction of the actual IUT II theta values, Gaussian monoids,
  splitting monoids, \(F_{\mathrm{mod}}^\times\)-actions, and Frobenioids behind
  the present degree/log-volume shadows;
